# Jet Lag: Hide and Seek - Game Rules Reference

This document contains the official game rules from the Jet Lag: Hide and Seek rulebook, with Stillwater-specific adaptations noted.

---

## Table of Contents

1. [Game Overview](#game-overview)
2. [Game Sizes](#game-sizes)
3. [Question Categories](#question-categories)
4. [Question Lists](#question-lists)
5. [Card Types](#card-types)
6. [Stillwater Adaptations](#stillwater-adaptations)

---

## Game Overview

### Basic Flow

1. One player (hider) uses public transit to reach a hiding spot during the hiding period
2. Other players (seekers) work together to find them by asking questions
3. Questions are chosen from 6 categories, each with different costs (cards for the hider)
4. Hider must answer truthfully within time limits
5. For each question answered, hider draws cards from their deck
6. Once hider is found, role passes to next player
7. Winner is the player who remained hidden the longest

### Player Requirements

- Minimum: 2 players
- Recommended maximum: 4 players
- Larger groups should form teams that act as a single player

---

## Game Sizes

| Size | Area Type | Duration | Stations | Hiding Period | Hiding Zone Radius |
|------|-----------|----------|----------|---------------|-------------------|
| **Small (S)** | Single town | 4-8 hours | 30-100 | 30 minutes | 1/4 mile (400m) |
| **Medium (M)** | Major city | ~1 day | 100-500 | 60 minutes | 1/4 mile (400m) |
| **Large (L)** | Region/country | 2-4 days | 500+ | 180 minutes | 1/2 mile (800m) |

**Stillwater uses Small (S) game size.**

---

## Question Categories

There are 6 question categories. Each has a specific format, card cost (draw/keep), and response time.

| Category | Format | Draw | Keep | Response Time |
|----------|--------|------|------|---------------|
| **Matching** | "Is your nearest _ the same as my nearest _?" | 3 | 1 | 5 min |
| **Measuring** | "Compared to me, are you closer to or further from _?" | 3 | 1 | 5 min |
| **Radar** | "Are you within _ of me?" | 2 | 1 | 5 min |
| **Thermometer** | "After traveling _, am I hotter or colder?" | 2 | 1 | 5 min |
| **Photo** | "Send us a photo of _" | 1 | 1 | 10 min (S/M), 20 min (L) |
| **Tentacle** | "Within _ of me, which _ are you nearest to? (You must also be within _)" | 4 | 2 | 5 min |

### Card Costs Explained

- **Draw**: How many cards the hider draws from the deck after answering
- **Keep**: How many of those drawn cards the hider keeps (discards the rest)
- Higher-value questions (more info for seekers) = more cards for hider

### Response Time Rules

- Hider must answer within the response time
- Photo questions get extra time due to photo requirements
- Timer starts when seekers submit the question

---

## Question Lists

### Matching Questions (22 total)

Format: **"Is your nearest _ the same as my nearest _?"**

All Matching questions are available in all game sizes.

#### Transit
| Question |
|----------|
| Is your nearest **commercial airport** the same as my nearest commercial airport? |
| Is your nearest **transit line** the same as my nearest transit line? |
| Is your nearest **station's name length** the same as my nearest station's name length? |
| Is your nearest **street or path** the same as my nearest street or path? |

#### Administrative Divisions
| Question | Notes |
|----------|-------|
| Is your nearest **1st admin division (state)** the same as mine? | |
| Is your nearest **2nd admin division (county)** the same as mine? | |
| Is your nearest **3rd admin division (city/municipality)** the same as mine? | |
| Is your nearest **4th admin division (quadrant)** the same as mine? | *Stillwater custom* |

*Stillwater quadrant definition: East-West divided by Husband St, North-South divided by McElroy Rd*

#### Natural
| Question |
|----------|
| Is your nearest **mountain** the same as my nearest mountain? |
| Is your nearest **landmass** the same as my nearest landmass? |
| Is your nearest **park** the same as my nearest park? |

#### Places of Interest
| Question | Notes |
|----------|-------|
| Is your nearest **amusement park** the same as mine? | |
| Is your nearest **zoo** the same as mine? | |
| Is your nearest **aquarium** the same as mine? | |
| Is your nearest **golf course** the same as mine? | |
| Is your nearest **museum** the same as mine? | |
| Is your nearest **movie theater** the same as mine? | |
| Is your nearest **restaurant** the same as mine? | *Stillwater custom* |

#### Public Utilities
| Question | Notes |
|----------|-------|
| Is your nearest **hospital** the same as mine? | |
| Is your nearest **library** the same as mine? | |
| Is your nearest **foreign consulate** the same as mine? | |
| Is your nearest **school** the same as mine? | *Stillwater custom* |

---

### Measuring Questions (20 total)

Format: **"Compared to me, are you closer to or further from _?"**

All Measuring questions are available in all game sizes.

#### Transit
| Question |
|----------|
| Compared to me, are you closer to or further from **a commercial airport**? |
| Compared to me, are you closer to or further from **a high speed train line**? |
| Compared to me, are you closer to or further from **a rail station**? |

#### Borders
| Question |
|----------|
| Compared to me, are you closer to or further from **an international border**? |
| Compared to me, are you closer to or further from **a 1st admin division border (state)**? |
| Compared to me, are you closer to or further from **a 2nd admin division border (county)**? |

#### Natural
| Question |
|----------|
| Compared to me, are you closer to or further from **sea level**? |
| Compared to me, are you closer to or further from **a body of water**? |
| Compared to me, are you closer to or further from **a coastline**? |
| Compared to me, are you closer to or further from **a mountain**? |
| Compared to me, are you closer to or further from **a park**? |

#### Places of Interest
| Question |
|----------|
| Compared to me, are you closer to or further from **an amusement park**? |
| Compared to me, are you closer to or further from **a zoo**? |
| Compared to me, are you closer to or further from **an aquarium**? |
| Compared to me, are you closer to or further from **a golf course**? |
| Compared to me, are you closer to or further from **a museum**? |
| Compared to me, are you closer to or further from **a movie theater**? |

#### Public Utilities
| Question |
|----------|
| Compared to me, are you closer to or further from **a hospital**? |
| Compared to me, are you closer to or further from **a library**? |
| Compared to me, are you closer to or further from **a foreign consulate**? |

---

### Radar Questions (10 total)

Format: **"Are you within _ of me?"**

All Radar questions are available in all game sizes.

| Distance | Notes |
|----------|-------|
| 0.2 miles | *Tweaked from original - Google Maps doesn't do 100ths* |
| 0.5 miles | |
| 1 mile | |
| 3 miles | |
| 5 miles | |
| 10 miles | |
| 25 miles | |
| 50 miles | |
| 100 miles | |
| Custom | *Seekers choose the distance* |

---

### Thermometer Questions (5 total)

Format: **"After traveling _, am I hotter or colder?"**

The thermometer tracks the starting and ending points of seekers to determine if they got closer ("hotter") or further ("colder") from the hider.

| Distance | Available In | Notes |
|----------|--------------|-------|
| 0.2 miles | All sizes | *Stillwater custom* |
| 0.5 miles | All sizes | |
| 3 miles | All sizes | |
| 10 miles | Medium & Large | |
| 50 miles | Large only | |

**For Stillwater (Small): 3 thermometer options available**

---

### Photo Questions (18 total)

Format: **"Send us a photo of _"**

Response time: 10 minutes (Small/Medium), 20 minutes (Large)

#### All Game Sizes (6 questions)

| Photo Request | Instructions |
|---------------|--------------|
| A tree | Must include entire tree |
| The sky | Place phone on ground and shoot directly up |
| You (selfie) | Selfie mode, arm parallel to the ground, fully extended |
| Widest street | Must include both sides of the street |
| Tallest structure in your sightline | Tallest from your current perspective. Must include top and both sides. Top must be in top 1/2 of frame |
| Any building visible from station | Stand directly outside transit station entrance. If multiple entrances, you may choose. Must include roof, both sides, top of building in top 1/2 of frame |

#### Medium & Large Only (8 additional questions)

| Photo Request | Instructions |
|---------------|--------------|
| Tallest building visible from station | Tallest from your perspective. Stand outside transit station entrance. Include roof, both sides, top in top 1/2 of frame |
| Trace nearest street/path | Street/path must be visible on mapping app. Trace intersection to intersection |
| Two buildings | Must include bottom and up to four stories |
| Restaurant interior | No zoom. Take picture through window from outside |
| Train platform | Must include a 5' x 5' section with three distinct elements |
| Park | No zoom, phone perpendicular to ground. Stand 5' from any obstruction |
| Grocery store aisle | No zoom. Stand at end of aisle. Shoot directly down aisle |
| Place of worship | Must include a 5' x 5' section with three distinct elements |

#### Large Only (4 additional questions)

| Photo Request | Instructions |
|---------------|--------------|
| 1/2 mile of streets traced | Must be continuous, including 5 turns, no doubling back. Send N-S oriented. Streets must appear on mapping app |
| Tallest mountain visible from station | Tallest from your perspective. Max 3x zoom. Top of mountain in top 1/2 of frame |
| Biggest body of water in your hiding zone | Max 3x zoom. Include either both sides of body of water or the horizon |
| Five buildings | Must include bottom & up to four stories |

**For Stillwater (Small): 6 photo options available**

---

### Tentacle Questions (8 total)

Format: **"Of all the [location type] within [distance] of me (the seeker), which are you (the hider) closest to?"**

**Important rule:** If the hider is NOT within [distance] of the seekers, they do not have to answer the question. This means Tentacle questions only work when the hider is relatively close to the seekers.

**Note: Tentacle questions are NOT available for Small game size.**

#### Medium & Large (1 mile distance)

| Location Type |
|---------------|
| Museums |
| Libraries |
| Movie theaters |
| Hospitals |

#### Large Only (15 miles distance)

| Location Type |
|---------------|
| Metro lines |
| Zoos |
| Aquariums |
| Amusement parks |

**For Stillwater (Small): 0 tentacle options available**

---

## Card Types

The hider's deck contains 3 types of cards (99 cards total).

### 1. Time Bonus Cards (55 cards)

- Add minutes to hiding duration when hider is found
- Only count if they are in hand at the end of the round
- Cannot be played, only discarded
- Values scale by game size

| Quantity | Small | Medium | Large |
|----------|-------|--------|-------|
| 25x | 2 min | 3 min | 5 min |
| 15x | 4 min | 6 min | 10 min |
| 10x | 6 min | 9 min | 15 min |
| 3x | 8 min | 12 min | 20 min |
| 2x | 12 min | 18 min | 30 min |

### 2. Powerup Cards (21 cards)

| Qty | Powerup | Effect |
|-----|---------|--------|
| 4x | Veto Question | Play instead of answering a question. No answer is given, and no reward is earned. |
| 4x | Randomize Question | Play instead of answering a question. A new unasked question from the same category is chosen at random, which you answer instead. |
| 4x | Discard 1, Draw 2 | Discard one other card from your hand. Then, draw and keep two cards from the hider deck. |
| 4x | Discard 2, Draw 3 | Discard two other cards from your hand. Then, draw and keep three cards from the hider deck. |
| 2x | Draw 1, Expand 1 | Draw one card from the hider deck. For the rest of the round, you can hold one additional card in your hand. |
| 2x | Duplicate Another Card | Play this card as a copy of any other card in your hand. This may be used to duplicate a time bonus at the end of your round. |
| 1x | Move | Discard your hand and send the seekers the location of your transit station. Grants a new hiding period (S:10min, M:20min, L:60min) to establish a new hiding zone. Seekers are frozen and hiding timer is paused until the new hiding period concludes. Cannot be played during endgame. |

### 3. Curse Cards (23 cards)

- Played by hider against seekers
- Impose conditions, challenges, or restrictions on seekers
- Many curses block question-asking until cleared
- **Rule:** Never more than one curse blocking questions or transit at a time

#### Curse Card Details

| Curse | Effect | Casting Cost |
|-------|--------|--------------|
| Lemon Phylactery | Seekers must each find a lemon and affix it to their clothes/skin before asking another question. If a lemon stops touching a seeker, hider gets bonus (S:30min, M:45min, L:60min). Cannot be played during endgame. | Discard a powerup |
| Luxury Car | Take a photo of a car. Seekers must take a photo of a more expensive car before asking another question. | Photo of a car |
| Water Weight | Seekers must carry 2L of liquid per seeker for rest of run. Cannot ask questions until acquired. If lost/abandoned, hider gets bonus (S:30min, M:30min, L:60min). | Seekers within 1000ft of body of water |
| Spotty Memory | For rest of run, one random category disabled at all times. After each question, roll die to choose new disabled category. | Discard a time bonus |
| Zoologist | Take photo of wild animal. Seekers must photo a wild animal in same category (fish/bird/mammal/reptile/amphibian/bug) before asking another question. | Photo of an animal |
| Mediocre Travel Agent | Choose a place within (S:0.25mi, M:0.25mi, L:0.5mi) of seekers. They must go there, spend (S:5min, M:5min, L:10min), send 3 photos, and bring a souvenir. If souvenir lost, hider gets bonus (S:30min, M:45min, L:60min). | Destination must be further from hider than seekers' current location |
| Gambler's Feet | For next (S:20min, M:40min, L:60min), seekers must roll die before taking steps. They may take that many steps before rolling again. | Roll die; if even, curse has no effect |
| Egg Partner | Seekers must acquire an egg before asking another question. Egg is an official team member. If egg is cracked/abandoned before end of run, hider gets bonus (S:30min, M:45min, L:60min). Cannot be played during endgame. | Discard two cards |
| Drained Brain | Choose three questions in different categories. Seekers cannot ask those questions for rest of run. | Discard your hand |
| Cairn | Build a rock tower. Seekers must build one of same height before asking another question. Rocks must be found in nature and dispersed after. | Build a rock tower |
| Endless Tumble | Seekers must roll a die at least 100ft and land on 5 or 6 before asking another question. If they hit someone with the die, hider gets bonus (S:10min, M:20min, L:30min). | Roll die; if 5 or 6, curse has no effect |
| Hidden Hangman | Seekers must beat hider in hangman (5-letter word, 7 wrong guesses allowed). After (S:1, M:2, L:3) losses, wait 10min and curse clears. | Discard 2 cards |
| Unguided Tourist | Send seekers a Google Street View image from within 500ft. Without using internet, they must find it in real life before using transit or asking questions. | Seekers must be outside |
| Right Turn | For next (S:20min, M:40min, L:60min), seekers can only turn right at intersections. 180° allowed only at dead ends. | Discard a card |
| Bird Guide | Film a bird for up to (S:5min, M:10min, L:15min). Seekers must film a bird for same time or longer before asking another question. | Film a bird |
| U-Turn | Seekers must disembark at next station (if serviced by other transit within S:0.5hr, M:0.5hr, L:1hr). | Seekers must be heading wrong way (next station further from hider) |
| Jammed Door | For next (S:0.5hr, M:1hr, L:3hr), seekers must roll 2 dice (7+ to enter) before passing through any doorway. Failed doorways can be re-attempted after (S:5min, M:10min, L:15min). | Discard two cards |
| Overflowing Chalice | For next three questions, hider may draw (not keep) an additional card. | Discard a card |
| Ransom Note | Next question must be composed of cut-out words/letters from printed material, at least 5 words. | Spell out "ransom note" as a ransom note |
| Impressionable Consumer | Seekers must enter a location or buy a product they saw advertised (ad must be 100ft+ from product/location, not on device). | Seekers' next question is free |
| Labyrinth | Draw a solvable maze (up to S:10min, M:20min, L:30min). Seekers must solve it before asking another question. | Draw a maze |
| Urban Explorer | For rest of run, seekers cannot ask questions when on transit or in transit stations. | Discard 2 cards |
| Distant Cuisine | Find restaurant in your zone serving food from specific foreign country. Seekers must visit restaurant serving food from equal/greater distance country before asking another question. | You must be at the restaurant |
| Bridge Troll | Seekers must ask their next question from under a bridge. | Seekers must be at least (S:1mi, M:5mi, L:30mi) from you |

### Hand Limit

- Default hand limit: 6 cards
- Some powerup cards increase hand limit

### Deck Summary

| Card Type | Count |
|-----------|-------|
| Time Bonus | 55 |
| Powerup | 21 |
| Curse | 23 |
| **Total** | **99** |

---

## Stillwater Adaptations

### Game Configuration

- **Game Size:** Small (S)
- **Transit System:** OSU bus routes
- **Hiding Period:** 30 minutes
- **Hiding Zone Radius:** 1/4 mile centered on bus stop

### Custom Questions Added

| Category | Question | Notes |
|----------|----------|-------|
| Matching | 4th admin division (quadrant) | E-W: Husband St, N-S: McElroy Rd |
| Matching | Restaurant | Added to Places of Interest |
| Matching | School | Added to Public Utilities |
| Thermometer | 0.2 miles | Shorter distance for small game area |

### Questions Available for Stillwater

| Category | Count |
|----------|-------|
| Matching | 22 |
| Measuring | 20 |
| Radar | 10 |
| Thermometer | 3 |
| Photo | 6 |
| Tentacle | 0 |
| **Total** | **61** |

---

## Question Rules Summary

1. **One at a time:** Seekers must wait for the current question to be answered before asking another
2. **Truthful answers:** Hider must answer each question truthfully
3. **Internet allowed:** Hider may use the internet to answer EXCEPT Google Street View and similar services
4. **End game restriction:** Once seekers enter hiding zone, hider cannot fulfill Photo or Oddball requests
5. **Re-asking:** Questions cannot be re-asked unless seekers pay double cost (or hider used Veto)

---

*Reference compiled from official Jet Lag: Hide and Seek rulebook*
*Last updated: January 2026*
